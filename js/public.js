...
// 解密
decodeURIComponent('http%3A%2F%2Flocalhost%3A9527%2F%23%2Fbanner%2Flist%3FuserId%3D75')
// http://localhost:9527/#/banner/list?userId=75

// 加密
encodeURIComponent('http://localhost:9527/#/banner/list?userId=75')
// http%3A%2F%2Flocalhost%3A9527%2F%23%2Fbanner%2Flist%3FuserId%3D75



changeNum(val) {
    var oldVal = this.data.virtualBalance;
    var newVal = val || 0;
    var countDown = oldVal > newVal;
    var duration = 500;
    ...
    this.timer = setInterval( () => {
        ...
        if (countDown) {
            // 数字变小
            speed = oldVal - (oldVal - newVal) * (progress / duration);
        } else {
            // 数字变大
            speed = oldVal + (newVal - oldVal) * (progress / duration);
        }
